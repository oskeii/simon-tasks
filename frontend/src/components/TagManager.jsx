...
const TagManager = () => {
  ...
  const deleteTag = async (tagId) => {
    if (window.confirm('There are N tasks in this tag\n Are you sure you want to delete this tag?')) {
        ...
        if (tagId === editingId) { toggleForm() }
    }
  };

  const toggleForm = (tag=null) => {
    if (tag) { // open form with data to edit
        setEditingId(tag.id);
        setFormData(tag);  
        setShowForm(true)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (editingId) {
            const response = await apiService.tags.update(editingId, formData);
  ...
  return (
    <div className='tag-manager'>
        ...
        {tags.length === 0 ? (
            <p>No tags created yet. Create one to start organizing your tasks!</p>
        ) : (
                    ...
                    {tags.map(tag => (
                        <li key={tag.id} className='tag-item'>
                            ...
                            <div className='tag-actions'>
                                <button onClick={() => toggleForm(tag)}>Edit</button>
