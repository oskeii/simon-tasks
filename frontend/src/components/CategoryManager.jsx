...
const CategoryManager = () => {
    ...
    const deleteCat = async (catId) => {
        if (
            window.confirm(
                'There are N tasks in this category\n Are you sure you want to delete this category?'
            )
        ) {
            try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingId) {
                const response = await apiService.categories.update(
                    editingId,
                    formData
                );
    ...
    return (
        <div className="category-manager">
            ...
            {categories.length === 0 ? (
                <p>
                    No categories created yet. Create one to start organizing
                    your tasks!
                </p>
            ) : (
